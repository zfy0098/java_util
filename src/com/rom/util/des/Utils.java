...
public class Utils {
	...
	public static HashMap<String, String> getFields(String data) {
		...
		try {
			...
			if ("9F".equalsIgnoreCase(tlv_tag)
					|| "5F".equalsIgnoreCase(tlv_tag)
					|| "BF".equalsIgnoreCase(tlv_tag)
					|| "DF".equalsIgnoreCase(tlv_tag)) {
			...
			String tlv_value;
			if (first_bit == 0) {
				tlv_value = data.substring(tlv_tag.length() + 2,
						tlv_tag.length() + 2 + tlv_len);
			} else {
				tlv_value = data.substring(
						tlv_tag.length() + 2 + tlv_l.length(), tlv_tag.length()
								+ 2 + tlv_l.length() + tlv_len);
			}
